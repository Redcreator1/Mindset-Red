...
**Extension VS Code** (`editors/vscode/`, publiée sur le VS Code Marketplace
sous `mindset-ctx.mindset-ctx`) : palette de commandes ("mindset-ctx: Generate Context Files", "… Index
...
- [x] v0.19 — **durcissement sécurité** après une revue complète du code : cookie de session passé en `SameSite=Strict` (le `Lax` initial laissait une fenêtre CSRF sur les routes GET mutantes `/v1/team/*` et `/v1/checkout` depuis l'arrivée de l'auth par cookie) ; paramètre `state` OAuth ajouté au flux SSO WorkOS (nonce en cookie, vérifié au callback en temps constant — bloque le login-CSRF) ; extension VS Code 0.1.2 : `execFile` sans shell à la place d'`exec` et réglages passés en `scope: "machine"` (un repo malveillant ne peut plus planter une commande via `.vscode/settings.json`), défaut `npx mindset-ctx` maintenant que le paquet est sur npm.
- [x] v0.20 — **blog natif** (`GET /blog`, `/blog/:slug`) plutôt qu'un embed X/Twitter tiers sur le site : zéro script externe, zéro tracking tiers, visuels de couverture en SVG inline (pas d'image à héberger séparément) dans la même palette que le reste du site. Premier article : présentation de mindset-ctx. Parité Node + Worker.
- [x] v0.21 — **balises Open Graph / Twitter Card** sur `/`, `/docs`, `/pricing`, `/blog` et `/blog/:slug` : un lien partagé sur X/Slack/Discord affiche maintenant un vrai aperçu (titre, description, image) au lieu d'une URL nue. Image de prévisualisation (1200×630) servie par `GET /og-image.png`, encodée en base64 dans `src/og-image.ts` plutôt qu'hébergée comme fichier séparé — fonctionne à l'identique sur le serveur Node et sur le Worker Cloudflare (qui n'a pas de système de fichiers), sans dépendance tierce. `og:image`/`og:url` ne sont émis que si un `baseUrl` est configuré (sinon on n'imprime pas une URL relative cassée, invalide pour Open Graph).
- [x] v0.22 — **favicon** (`/favicon.svg`, même logo que partout ailleurs, redirection depuis `/favicon.ico`), **page 404 stylée** (`/v1/*` garde du JSON pour les clients API, tout le reste voit une vraie page) et **robots.txt/sitemap.xml** pour l'indexation. Bug réel trouvé et corrigé en construisant la 404 : sur les deux runtimes, une route inconnue tombait dans la porte d'authentification *avant* d'atteindre le routage — un visiteur non-authentifié sur une URL mal tapée recevait "401 unauthorized" au lieu de "404 not found" dès que l'auth tenant était configurée (le cas permanent du Worker hébergé). Corrigé en sortant la 404 du chemin non-`/v1/*` avant la porte d'auth, sur `server.ts` et `worker/index.ts`.
...
- [x] v0.30 — **thème clair sur tout le site** : navbar sombre + corps clair + titre géant audacieux, aligné à gauche (structure inspirée d'un design courant type NVIDIA Developer, mais avec l'accent bleu de mindset-ctx, pas une couleur de marque tierce). Appliqué de façon cohérente sur toutes les pages (accueil, docs, tarifs, blog, sécurité, legal, support, dashboard) — pas juste la page d'accueil, pour éviter un flash de thème sombre en naviguant. Les blocs de code (`pre`) restent volontairement sombres pour le contraste (même motif que GitHub/Stripe). Aucun changement de logique, uniquement CSS/couleurs — vérifié visuellement page par page avant livraison. 155/155.
- [x] v0.31 — **refonte du footer** : bandeau sombre en colonnes (Produit / Ressources / Légal) + barre de copyright, sur le modèle structurel d'un footer SaaS courant (ex. NVIDIA Developer) mais avec uniquement des liens réels du site — pas d'icônes de réseaux sociaux ni de formulaire newsletter pour des canaux qui n'existent pas encore. Appliqué à `home.ts` (partagé par accueil/docs/sécurité/blog/legal/support), `pricing.ts` et `dashboard.ts`. CSS uniquement, aucun changement de logique. 155/155.
- [x] v0.32 — **refonte de `/docs`** : bandeau titre "Centre de documentation" en grand gras (même langage typographique que la home), barre de recherche client-side (filtre les cartes par texte, sans backend) et onglets "Parcourir par catégorie" en ancres réelles vers chaque section — structure inspirée d'un centre de doc SaaS courant (ex. NVIDIA Developer), sans bannière sombre ni image de marque tierce, cohérent avec le thème clair du site. Grille de cartes de doc inchangée en dessous. CSS + un peu de JS vanilla, aucune route serveur ajoutée. 155/155.
